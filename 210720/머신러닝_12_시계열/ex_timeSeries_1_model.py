...
def make_model() :
    model = Sequential()
    model.add(SimpleRNN(10,input_shape=(3,1))) # 10은 그냥 중간층이기때문에 출력층보다 많은 아무숫자 다됨 
    model.add(Dense(1))  #, activation="linear" #숫자예측 relu아니면 linear쓰는데 아무것도 없으면 linear를 뜻함
    model.compile(loss='mse' , optimizer='sgd')
    model.summary() #그냥 뽑기위해(?)
    return model
